...
/**
 * BrazilCpfCheckInput Class Doc Comment
 *
 * @category Class
 * @package  Trinsic\Api
 * @author   OpenAPI Generator team
 ...
 */
class BrazilCpfCheckInput implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * @var string
     */
    protected static $openAPIModelName = 'BrazilCpfCheckInput';
    ...
    public function setCpfNumber($cpf_number)
    {
        ...
        if ((mb_strlen($cpf_number) > 11)) {
            throw new \InvalidArgumentException('invalid length for $cpf_number when calling BrazilCpfCheckInput., must be smaller than or equal to 11.');
        }
        if ((mb_strlen($cpf_number) < 11)) {
            throw new \InvalidArgumentException('invalid length for $cpf_number when calling BrazilCpfCheckInput., must be bigger than or equal to 11.');
        }
        if ((!preg_match("/^\\d*$/", ObjectSerializer::toString($cpf_number)))) {
            throw new \InvalidArgumentException("invalid value for \$cpf_number when calling BrazilCpfCheckInput., must conform to the pattern /^\\d*$/.");
        }
     ...
     * Sets selfie_image
     *
     * @param string|null $selfie_image The raw bytes of the selfie image collected from the user.
     *
     * @return self
     */
    ...
    /**
     * Gets selfie_image_content_type
     *
     * @return string|null
     */
    public function getSelfieImageContentType()
    {
        return $this->container['selfie_image_content_type'];
    }

    /**
     * Sets selfie_image_content_type
     *
     * @param string|null $selfie_image_content_type The MIME Type of the file contained in `SelfieImage`.              Must be one of `image/jpeg` or `image/png`.
     *
     * @return self
     */
    public function setSelfieImageContentType($selfie_image_content_type)
    {
        if (is_null($selfie_image_content_type)) {
            array_push($this->openAPINullablesSetToNull, 'selfie_image_content_type');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('selfie_image_content_type', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
        }
        $this->container['selfie_image_content_type'] = $selfie_image_content_type;
