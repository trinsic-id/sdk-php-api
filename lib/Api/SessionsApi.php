...
class SessionsApi
{
    ...
    {
        list($response) = $this->exchangeResultsKeyWithHttpInfo($session_id, $exchange_results_key_request, $contentType);
        return $response;
    }

    /**
     * Operation exchangeResultsKeyWithHttpInfo
     *
     * Exchange a Results Access Key for Identity Data
     *
     * @param  string $session_id (required)
    ...
    /**
     * Operation exchangeResultsKeyAsync
     *
     * Exchange a Results Access Key for Identity Data
     *
     * @param  string $session_id (required)
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function exchangeResultsKeyAsync($session_id, $exchange_results_key_request = null, string $contentType = self::contentTypes['exchangeResultsKey'][0])
    {
        return $this->exchangeResultsKeyAsyncWithHttpInfo($session_id, $exchange_results_key_request, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    /**
     * Operation exchangeResultsKeyAsyncWithHttpInfo
     *
     * Exchange a Results Access Key for Identity Data
     *
     * @param  string $session_id (required)
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function exchangeResultsKeyRequest($session_id, $exchange_results_key_request = null, string $contentType = self::contentTypes['exchangeResultsKey'][0])
    {

        // verify the required parameter 'session_id' is set
        if ($session_id === null || (is_array($session_id) && count($session_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $session_id when calling exchangeResultsKey'
            );
        }



        $resourcePath = '/api/v1/sessions/{sessionId}/results/exchange';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // for model (json/xml)
        if (isset($exchange_results_key_request)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($exchange_results_key_request));
            } else {
                $httpBody = $exchange_results_key_request;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
